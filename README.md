...
```sh
npm install
# credentials must be for the account mapped to the env label:
npx cdk bootstrap -c env=dev --profile dev    # once per account/region
npx cdk deploy   -c env=dev --profile dev
```

Deploy one environment at a time; repeat with `-c env=qa`, `-c env=prod` and the matching AWS profile. CDK refuses to deploy if your credentials don't match the account pinned in `environments.json`.
